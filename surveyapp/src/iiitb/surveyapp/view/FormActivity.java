...
public abstract class FormActivity extends Activity
{
	...
	public static final LayoutParams defaultLayoutParams = new LinearLayout.LayoutParams( LayoutParams.FILL_PARENT, 
	...
	public void generateForm( String data )
	{
		...
		_container.setBackgroundColor(Color.parseColor("#33b5e5"));
	...
	protected JSONObject submit(int surveyid, int participantId)
	{
		FormWidget widget;
		JSONObject data = new JSONObject();
		try {
		...
		try{
			...
			{	
				...
				data.put( "question_id",Integer.toString(widget.getPriority()));
				data.put("answer", widget.getValue() );
			}
		} catch( JSONException e )
		{
			success = false;
